...
//        }
    ...
    public function showrooms(Request $request)
    {
        ...
        if ($request->district != null) {
            ...
            $db_showroomList = $this->post->self()->where('categories', 651)->orderBy('id', 'DESC')->where('shop_type', '!=', 'Chatbuy')->get();
        }
...
//        }
    ...
    public function allTerms(Request $request)
    {
        ...
        $terms = Cache::remember('common-all-terms', 86400, function () use ($request) {
            ...
            $terms = [];
            $alltarms = $this->terms->self()->where('type', "category")->where('is_published', 1)->orderBy('serial', 'ASC')->get();
    ...
    public function getSiteMap()
    {
        ...
        $products = $this->product->self()->select("id", "product_code", "title", "seo_url", "description", "stock_status", "local_selling_price", "local_discount")->with('firstImage')->where('is_active', 1)->get();
