...
//        dd(1);
    ...
    private function storeRecentViews($self_token, $product_id)
    {
        ...
        $product_ids = Cache::remember($self_views_key, 18080, function () use ($product_ids) {
...
//            dd($data->variation_image);
            ...
            $img = $img ? 'https://admin.masterleatherbd.com/' . $img : false;
...
//                    dd($variationImages);
                    ...
                    foreach ($variationImages as $im) {
                        $mkVariationImgArray [] = [
                            'id' => $im->id,
                            'url' => 'https://admin.masterleatherbd.com/' . $im->full_size_directory,
                        ];
                    }
                    //end
                    ...
                    foreach ($jsonDecoded as $key => $data) {
                        if ($data->show_as == 'Image') {
                            ...
                            $imageLink = 'https://admin.masterleatherbd.com/' . $imageLink;
                            $data = (object)[
                                'link' => $imageLink,
                                'value' => $data->value,
                                'show_as' => 'Image',
