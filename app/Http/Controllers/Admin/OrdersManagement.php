...
//        dd($r->all());
        ...
        $order_master_create = \App\Models\OrdersMaster::create($orderMaster);
        $products = [];
        if ($r->product) {
            foreach ($r->product as $key => $product) {
//                $product = $r->product[$key];
                $product_info = \App\Models\Product::Where(['id' => $product['product_id']])->first();
//                dd($product_info);
                $item = [
...
//                    'product_fabric_id' => $item->fabric_id??null,
            ...
            $orderDetails = \App\Models\OrdersDetail::insert($products);
            ...
            //End
            $subject = 'Thank you for ordering from Master Leather BD!';
            $address = $r->email;
