...
class ShopController extends Controller
{
    ...
    public function search_product(Request $request)
    {
        ...
        if (!empty($category->id)) {
            ...
            //dd($settings);
            return view('frontend.products.search_results', compact('products'))
                ->with([
                    'settings' => $settings,
                    'posts' => $posts,
                    'widgets' => $widgets,
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if ($add_cat) {
            ...
            return response()->json(['data' => $html, 'total_amount' => number_format($totalprice), 'total_qty' => $total_qty, 'report' => 'Yes']);
        } else {
...
//        }
    ...
    public function place_order($data, $rand, $secret_key)
    {
        ...
        if ($data['payment_method']['payment_method'] == 'cash_on_delivery') {
            ...
            //dd($data['payment_method']['payment_method']);
            //dd('This will setup later');
...
//                    redirect('/checkout/success?order_id=' . $orderplacing->id . '&random_code=' . $rand . '&secret_key=' . $secret_key)->send();
    ...
    public function success(Request $request)
    {
        ...
        if (Auth::check()) {
                        ...
                        try {
                            ...
                            if ($order_detail_created != null && $orders_master_attributes) {
                                ...
                                if ($array['OrderStatus'] == 'APPROVED') {
                                    $update_product = $this->ordersmaster->update($orders_master->id, ['order_status' => 'APPROVED', 'pur']);
                    ...
                    if (!empty($request->tran_id)) {
                        ...
                        if ($orders_master->order_status == 'Pending') {
                            ...
                            if ($validation == true) {
                                ...
                                $update_product = $this->ordersmaster->update($request->value_a, ['order_status' => 'Processing', 'trans_id' => $tran_id]);
            ...
            $user_id = $data['user_details']['user_id'][0];
            //dd($user_id);
            $orders_master = $this->ordersmaster->getById($request->get('order_id'));
...
//            $html .= '</button>';
    ...
    public function main_search_form(Request $request)
    {
        ...
        if ($cats != null) {
            ...
            $categories = $cats->toArray();
            //dd($settings);
            return view('frontend.products.search_results_key', compact('products'))
                ->with(['settings' => $settings,
                    'posts' => $posts,
                    'widgets' => $widgets,
