...
function prepare_sslcommerz($cart, $user_data, $trackcode, $secret_key, $order_id)
{
    ...
    $fields = array(
        ...
        'cus_email' => $user_data->email != '' ? $user_data->email : 'customer@masterleatherbd.com',
...
function payment_return_url()
{
    ...
    if ($status == 'success') {
        ...
        //$customer_message = "Order has been placed successfully on Master Leather\nOrder No: " . $result->id . "\nHotline: 01757757235";
        $customer_message = "Order has been placed successfully on Master Leather BD\nOrder No: " . $result->id;
        sendSms($customer_message, $result->contuct_number);
        sendSms($customer_message, $result->alternative_mobile);

        //Mobile MSG admin
        $message = "New Order has been placed successfully on Master Leather BD\nOrder No: " . $result->id;
